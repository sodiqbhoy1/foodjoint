              ...
              <div className="p-4 flex-1 overflow-auto space-y-4">
                {items.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center text-gray-600">
                    ...
                    <p className="text-sm mt-2">Add delicious items from the menu to get started.</p>
                  </div>
                ) : (
                  items.map((it) => {
                      ...
                      <div key={it.key} className="flex items-center gap-3">
                        <div className="w-16 h-16 rounded-md overflow-hidden bg-gray-100 flex-shrink-0">
                          {img ? (
                            ...
                            <Image src={img} alt={it.title} width={64} height={64} className="object-cover w-full h-full" />
                          ) : (
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between">
                            ...
                            <button onClick={() => remove(it.key)} aria-label={`Remove ${it.title}`} className="text-red-500 p-2 hover:bg-red-50 rounded">
                              <FiTrash />
                            </button>
                          </div>
                          ...
                          <div className="mt-2 flex items-center gap-3">
                            <div className="inline-flex items-center rounded-md bg-gray-100 border border-gray-200">
                              <button onClick={() => update(it.key, Math.max(1, it.qty - 1))} className="px-3 py-1 text-gray-700">−</button>
